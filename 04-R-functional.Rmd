...
## Fitting lots of models (3/`r n_slides`) {.smallcode}
...
make_forms <- function(y, Xs, n_Xs) {
  ...
  # Change to names:
  names_ <- map(combs, function(x) poss_Xs[x])
...
## Fitting lots of models (5/`r n_slides`) {.smaller}

Put both steps together:

```{r lotsa_models3}
fit_models <- function(Ys, Xs, n_Xs) {
  forms <- map(Ys, make_forms,
                  Xs = Xs, n_Xs = n_Xs)
  forms <- c(forms, recursive = TRUE)
  lms <- map(forms, single_mod)
  return(lms)
}
